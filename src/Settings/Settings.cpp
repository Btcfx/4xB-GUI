...
const char DEFAULT_WALLET_FILE_NAME[] = "4xbitcoinwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "4xbitcoinpool.com:3333";
m_defaultPoolList << "pit-b.cryptoknight.cc:5661";
m_defaultPoolList << "xao.mine2gether.com:1117";
m_defaultPoolList << "sg.4xbitcoin.cryptonight.me:3333";
...
m_defaultPoolList << "4xbitcoin.hashat.me:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("4xbitcoinwallet.cfg"));
  ...
  if (cfgFile.open(QIODevice::ReadOnly)) {
   ...
   optimizationObject.insert(OPTION_WALLET_OPTIMIZATION_FUSION_TARNSACTIONS_IS_VISIBLE, true);
    m_settings.insert(OPTION_NODE_REMOTE_RPC_URL, QString("rpc.4xbitcoinproject.org:1811"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("4xbitcoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("4xbitcoinwallet.desktop");
  ...
  res = autorunSettings.contains("4xbitcoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("4xbitcoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.4xbitcoin.4xbitcoin scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=4xbitcoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("4xbitcoinWallet", appPath);
    } else {
      autorunSettings.remove("4xbitcoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\4xbitcoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:4xbitcoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\4xbitcoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\4xbitcoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("4xbitcoinwallet.cfg"));
