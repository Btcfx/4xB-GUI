...
const char BYTECOIN_URI_SCHEME_NAME[] = "4xbitcoin";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("4xbitcoinwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("4xbitcoinwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] 4xbitcoin wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "4xbitcoin wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have 4xbitcoind with non-default RPC port, you should terminate it and relaunch 4xbitcoinWallet\n"
      "or\n"
      "Set the Local deamon required port in 4xbitcoinWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
